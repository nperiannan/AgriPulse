...
param(
    ...
    [switch]$DirectOta,  # Upload directly via espota to agripulse.local (LAN only)
...
$OtaDir   = '/opt/agripulse/data/ota'  # Docker volume mount → /data/ota inside container
...
try {
    if ($DirectOta) {
        Write-Host "==> Building + uploading directly via espota to agripulse.local..." -ForegroundColor Cyan
        ...
        $ApiToken = $env:AGRIPULSE_TOKEN
        if (-not $ApiToken) {
            $cred = Get-Credential -Message "AgriPulse server login" -UserName "admin"
