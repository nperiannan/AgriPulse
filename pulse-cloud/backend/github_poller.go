...
// new pulse-hub release.  When a newer version is found the firmware
...
//
// Flow:
//   GitHub Releases (pulse-hub/vX.Y.Z)
...
const (
	ghReleasesURL  = "https://api.github.com/repos/nperiannan/AgriPulse/releases?per_page=20"
	ghPollInterval = 5 * time.Minute
)

// ghLastStagedVersion is the pulse-hub version most recently staged
...
// ghPollOnce fetches the latest pulse-hub release from GitHub and
...
func ghPollOnce() error {
	...
	req.Header.Set("User-Agent", "AgriPulse-Cloud/"+webVersion)
	...
	// ── 2. Find latest pulse-hub release ─────────────────────────────────
	var latestTag, assetURL string
	for _, r := range releases {
		if !strings.HasPrefix(r.TagName, "pulse-hub/") {
			continue
		}
	...
	if latestTag == "" {
		log.Printf("[GH-POLL] no pulse-hub release with firmware.bin found")
		return nil
	}

	// Strip "pulse-hub/v" prefix → bare semver e.g. "2.3.3"
	version := strings.TrimPrefix(latestTag, "pulse-hub/")
	version = strings.TrimPrefix(version, "v")

	if version == ghLastStagedVersion {
		log.Printf("[GH-POLL] pulse-hub v%s already staged — no action", version)
		return nil
	}

	log.Printf("[GH-POLL] new pulse-hub v%s found — downloading from GitHub...", version)
	...
	if staged > 0 {
		...
		log.Printf("[GH-POLL] pulse-hub v%s staged for %d device(s) — ESP32 will pick up on next OTA poll", version, staged)
	}
	return nil
}
